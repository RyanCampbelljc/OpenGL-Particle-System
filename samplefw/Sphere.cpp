...
void Sphere::render(const glm::mat4& proj, const glm::mat4& view) const
{
	glm::mat4 mWorld = glm::translate(glm::mat4(1.0f), m_pos);
	mWorld = glm::scale(mWorld, m_scale);
	m_pMat->SetUniform("projection", proj);
	m_pMat->SetUniform("view", view);
	m_pMat->SetUniform("world", mWorld);
	m_pMat->SetUniform("worldIT", glm::transpose(glm::inverse(mWorld)));
