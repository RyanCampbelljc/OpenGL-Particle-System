...
BlendEquation Material::s_currentBlendEquationRGB = BE_Add;
BlendEquation Material::s_currentBlendEquationA = BE_Add;
BlendMode Material::s_currentSrcFactorRGB = BM_One;
BlendMode Material::s_currentDstFactorRGB = BM_Zero;
BlendMode Material::s_currentSrcFactorA = BM_One;
BlendMode Material::s_currentDstFactorA = BM_Zero;
...
Material::Material(const std::string& name)
	...
	, m_depthTest(true)
	, m_depthWrite(true)
	, m_depthFunc(DF_LessEqual)
	, m_blendEnabled(false)
	, m_srcRGB(BM_One)
	, m_dstRGB(BM_Zero)
	, m_srcA(BM_One)
	, m_dstA(BM_Zero)
	, m_blendEquationRGB(BE_Add)
	, m_blendEquationA(BE_Add)
	, m_currVariant(0)
{
}
...
void Material::Apply()
{
	...
	// Set up blending
	{
		...
		if (m_blendEnabled) {
			if (m_blendEquationRGB != s_currentBlendEquationRGB || m_blendEquationA != s_currentBlendEquationA) {
				s_currentBlendEquationRGB = m_blendEquationRGB;
				s_currentBlendEquationA = m_blendEquationA;
				glBlendEquationSeparate(g_aBlendEquationMap[m_blendEquationRGB], g_aBlendEquationMap[m_blendEquationA]);
			}

			if (m_srcRGB != s_currentSrcFactorRGB || m_srcA != s_currentSrcFactorA || m_dstA != s_currentDstFactorA
				|| m_dstRGB != s_currentDstFactorRGB) {
				s_currentSrcFactorRGB = m_srcRGB;
				s_currentDstFactorRGB = m_dstRGB;
				s_currentSrcFactorA = m_srcA;
				s_currentDstFactorA = m_dstA;
				glBlendFuncSeparate(
					g_aBlendModeMap[m_srcRGB], g_aBlendModeMap[m_dstRGB], g_aBlendModeMap[m_srcA],
					g_aBlendModeMap[m_dstA]);
			}
		}
	}
}
